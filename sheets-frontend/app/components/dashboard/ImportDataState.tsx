...
type ImportSortingMode = 'file_order' | 'ascending' | 'descending';

interface ImportedStudent extends Student {
  email?: string;
...
export const ImportDataState: React.FC<ImportDataStateProps> = ({
  ...
  const [importSortingMode, setImportSortingMode] = useState<ImportSortingMode>('file_order');
    ...
    try {
      if (fileExtension === 'csv') {
        ...
        Papa.parse(file, {
          ...
          skipEmptyLines: true,
          complete: (results) => {
            processImportedData(results.data as ImportedRow[]);
          },
          error: () => {
  ...
  const processImportedData = (data: ImportedRow[]) => {
    if (data.length === 0) {
    ...
    // Columns to ignore (serial numbers, etc.)
    const ignoreColumns = ['sr no', 'sr. no', 's.no', 's no', 'sno', 'serial', 'serial no', '#', 'no'];

    // Identify custom columns
    const customColumnNames = columns.filter(col => {
      const colLower = col.toLowerCase().trim();
      if (col === nameCol) return false;
      if (col === rollNoCol) return false;
      if (col === emailCol) return false;
      if (ignoreColumns.some(ignore => colLower.includes(ignore.toLowerCase()))) return false;
      return true;
    });

    console.log('Detected custom columns:', customColumnNames);
    ...
    // Process students
    const students: ImportedStudent[] = data.map((row, index) => {
      ...
      if (emailCol && row[emailCol]) {
        student.email = String(row[emailCol]).trim();
      }

      // Add custom column values (stored temporarily by column NAME)
      if (customColumnNames.length > 0) {
        const customFields: Record<string, string> = {};
        customColumnNames.forEach(colName => {
          const value = row[colName];
          if (value !== undefined && value !== null && String(value).trim() !== '') {
            customFields[colName] = String(value).trim();
          }
        });
  ...
  const handleImportComplete = () => {
    ...
    // Extract unique custom column names from all students
    const customColumnNamesSet = new Set<string>();
    importedStudents.forEach(student => {
      const importedStudent = student as ImportedStudent;
      if (importedStudent.customFields) {
        Object.keys(importedStudent.customFields).forEach(key => {
          customColumnNamesSet.add(key);
        });
      }
    });

    // Create customColumns array with unique IDs
    const customColumns = Array.from(customColumnNamesSet).map((colName, index) => ({
      id: `custom-${Date.now()}-${index}`,
      name: colName,
      label: colName,  // ✅ Added this line
      type: 'text' as const
    }));


    console.log('Creating class with custom columns:', customColumns);

    // FIX: Map custom field data from column NAMES to column IDs
    const mappedStudents = importedStudents.map(student => {
      const mappedStudent: ImportedStudent = { ...student };
      const importedStudent = student as ImportedStudent;

      if (importedStudent.customFields) {
        // For each custom column, map data to use column.id as key
        customColumns.forEach(column => {
          const fieldValue = importedStudent.customFields?.[column.name];
          if (fieldValue !== undefined) {
            // Store using column.id (not column.name)
            mappedStudent[column.id] = fieldValue;
          }
        });

        // Remove the temporary customFields object
        delete mappedStudent.customFields;
      }

      return mappedStudent;
    });

    console.log('Mapped students sample:', mappedStudents[0]);
    ...
    const newClass: Class = {
      ...
      sorting_mode: importSortingMode,
    };
          ...
          <div className="max-w-md sm:mx-auto max-w-xl space-y-6 animate-fade-in">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-gray-200">
              <div className="text-center mb-6 sm:mb-8">
                <div className={`inline-flex p-3 sm:p-4 rounded-2xl ${selectedModeConfig.iconBg} mb-4 mx-auto`}>
                  <SelectedModeIcon className={`w-8 h-8 sm:w-10 sm:h-10 ${selectedModeConfig.iconColor}`} />
                </div>
          ...
          <div className="max-w-lg sm:max-w-4xl mx-auto space-y-6 animate-fade-in">
            <button
              onClick={() => {
                ...
                setError('');
                setImportedStudents([]);
                setImportSortingMode('file_order');
              }}
              className="flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors text-sm sm:text-base"
            >
              <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5" />
              ...
              {importedStudents.length === 0 ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white rounded-xl p-4 sm:p-6 border border-purple-200">
                    <h3 className="text-base sm:text-lg font-bold text-slate-900 mb-2">Sorting</h3>
                    <p className="text-xs sm:text-sm text-slate-600 mb-4">
                      Choose how students should be displayed in the attendance sheet.
                    </p>
                    <div className="space-y-2">
                      <label className="flex items-start gap-3 p-3 rounded-lg border border-slate-200 hover:border-emerald-300 cursor-pointer">
                        <input
                          type="radio"
                          name="import-sorting"
                          value="file_order"
                          checked={importSortingMode === 'file_order'}
                          onChange={() => setImportSortingMode('file_order')}
                          className="mt-1"
                        />
                        <div>
                          <p className="text-sm font-semibold text-slate-900">Import in file order</p>
                          <p className="text-xs text-slate-600">Keep the same order as your uploaded file.</p>
                        </div>
                      </label>
                      <label className="flex items-start gap-3 p-3 rounded-lg border border-slate-200 hover:border-emerald-300 cursor-pointer">
                        <input
                          ...
                          name="import-sorting"
                          value="ascending"
                          checked={importSortingMode === 'ascending'}
                          onChange={() => setImportSortingMode('ascending')}
                          className="mt-1"
                        />
                        <div>
                          <p className="text-sm font-semibold text-slate-900">Sort in ascending order</p>
                          <p className="text-xs text-slate-600">Sort students by roll number (low to high).</p>
                        </div>
                      </label>
                      <label className="flex items-start gap-3 p-3 rounded-lg border border-slate-200 hover:border-emerald-300 cursor-pointer">
                        <input
                          ...
                          name="import-sorting"
                          value="descending"
                          checked={importSortingMode === 'descending'}
                          onChange={() => setImportSortingMode('descending')}
                          className="mt-1"
                        />
                        <div>
                          <p className="text-sm font-semibold text-slate-900">Sort in descending order</p>
                          <p className="text-xs text-slate-600">Sort students by roll number (high to low).</p>
                        </div>
                      </label>
                    </div>
                  </div>
