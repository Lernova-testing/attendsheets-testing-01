...
const buildInitialSessions = (currentSessions: Session[]): Session[] => {
...
export const MultiSessionModal: React.FC<MultiSessionModalProps> = ({
  ...
  const [sessions, setSessions] = useState<Session[]>(() => buildInitialSessions(currentSessions));

  const toggleStatus = (index: number) => {
    setSessions(prevSessions => {
      const newSessions = [...prevSessions];
      const currentStatus = newSessions[index].status;
          ...
          {sessions.map((session, index) => (
            <div
              ...
              className={`bg-slate-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border-2 transition-all ${
                index === 0 ? 'border-emerald-300 bg-emerald-50/50' : 'border-slate-200'
              }`}
            >
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <span className="text-xs sm:text-sm font-semibold text-slate-900 truncate">{session.name}</span>
                  {index === 0 && (
