...
import { Calendar, BarChart3, Check, Sparkles, Linkedin, Menu, X, Instagram, QrCode } from 'lucide-react';
      ...
      {isMenuOpen && (
        <div className="md:hidden border-t border-emerald-200/60 bg-white/95 backdrop-blur-xl">
          <div className="px-4 py-4 flex flex-col gap-3">
            <Link href="/" className="text-md font-medium text-emerald-600 py-2" onClick={() => setIsMenuOpen(false)}>Home</Link>
            <Link href="/about" className="text-md font-medium text-slate-600 hover:text-emerald-600 transition-colors py-2" onClick={() => setIsMenuOpen(false)}>About Us</Link>
            <Link href="/contact" className="text-md font-medium text-slate-600 hover:text-emerald-600 transition-colors py-2" onClick={() => setIsMenuOpen(false)}>Contact</Link>
            <Link href="/auth" className="px-6 py-3 bg-emerald-600 text-white text-md font-medium rounded-lg hover:bg-emerald-700 text-center mt-2" onClick={() => setIsMenuOpen(false)}>Sign Up</Link>
          </div>
        </div>
      )}
    </nav>
    ...
    <section className="pt-24 sm:pt-28 md:pt-32 pb-12 sm:pb-16 md:pb-20 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
          <Link href="/auth" className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-emerald-600 text-white text-base sm:text-lg font-semibold rounded-xl hover:bg-emerald-700 transition-all shadow-lg hover:shadow-xl text-center">
            Start Tracking Now
          </Link>
          <Link href="/about" className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-white text-emerald-600 text-base sm:text-lg font-semibold rounded-xl border-2 border-emerald-200 hover:bg-emerald-50 transition-all text-center">
            Learn More
          </Link>
        </div>
      </div>
  ...
  const features = [
    {
      icon: QrCode,
      title: 'QR Marking',
      description: 'Allow students to mark attendance instantly by scanning a unique QR code. Simplify tracking and reduce manual entry errors.',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="border-t border-slate-800 pt-6 sm:pt-8 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-4">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <Link href="https://www.instagram.com/lernova.official" className="w-10 h-10 bg-slate-800 hover:bg-emerald-600 rounded-lg flex items-center justify-center transition-colors">
              <Instagram className="w-5 h-5" />
            </Link>
            <Link href="https://www.linkedin.com/company/lernova-official" className="w-9 h-9 sm:w-10 sm:h-10 bg-slate-800 hover:bg-emerald-600 rounded-lg flex items-center justify-center transition-colors">
