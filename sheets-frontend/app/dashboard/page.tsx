...
export default function DashboardPage() {
  ...
  const loadPendingDeviceRequests = async () => {
    try {
      ...
      if (response.ok) {
        const data = (await response.json()) as { requests?: Array<{ status?: string }> };
          ...
          prevClasses.map(cls =>
            cls.id === classId
              ? {
                  ...
                  ...normalizedClass,
                  sorting_mode:
                    normalizedClass.sorting_mode ??
                    cls.sorting_mode ??
                    (cls.enrollment_mode === 'import_data' ? 'file_order' : 'ascending'),
                }
              : cls
          )
        );
  ...
  const loadClassesFromBackend = async () => {
    try {
      ...
      const normalizedBackendClasses = backendClasses.map((cls) => ({
        ...cls,
        sorting_mode: cls.sorting_mode ?? (cls.enrollment_mode === 'import_data' ? 'file_order' : 'ascending'),
      }));

      if (normalizedBackendClasses.length > 0) {
        setClasses(normalizedBackendClasses);
  ...
  const handleClassSelect = (id: number) => {
    ...
    setShowSnapshot(false);
    // When entering a class view, switch back to single-month mode.
    setRangeStartMonth(currentMonth);
    setRangeStartYear(currentYear);
    setRangeEndMonth(currentMonth);
  ...
  const handleUpdateStudent = (studentId: number, field: string, value: unknown) => {
    if (!activeClassId) return;
            ...
            {!showImportState && classes.length > 0 && (
              <MonthYearSelector
                ...
                onRangeChange={({ startMonth, startYear, endMonth, endYear }) => {
                  ...
                  // Keep sheet views anchored to the range end month/year.
            ...
            <AttendanceSheet
              ...
              onUpdateClassData={(updatedClass) => {
                setClasses(prevClasses => {
                  ...
                  if (user) {
                    localStorage.setItem(`classes_${user.id}`, JSON.stringify(nextClasses));
                  }
