...
import { calculateStudentAttendanceForRange } from '@/lib/statisticsHelper';
...
interface ClassDetails {
  ...
  student_record: {
    ...
    attendance: Record<
      string,
      | { sessions?: Array<{ id: string; name: string; status: 'P' | 'A' | 'L' }>; status?: 'P' | 'A' | 'L'; count?: number }
      | 'P'
      | 'A'
      | 'L'
      | undefined
...
export default function StudentDashboard() {
    ...
    const checkDeviceTrust = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const isDeviceTrusted = trustedDevices.some((device: StudentDevice) => device.id === currentDeviceId);
  ...
  const loadClasses = async () => {
    try {
      ...
      if (response.ok) {
        const data = (await response.json()) as StudentClassesResponse;
        ...
        if (data.classes && data.classes.length > 0) {
          data.classes.forEach((cls, idx: number) => {
  ...
  const getClassRangeStats = (classInfo: ClassDetails) => {
    const attendance = classInfo.student_record?.attendance ?? {};
    const stats = calculateStudentAttendanceForRange(
      attendance,
      rangeStartMonth,
      rangeStartYear,
      rangeEndMonth,
  ...
  const shouldScrollClasses = filteredClasses.length > 3;

  const classStatsById = new Map(classes.map((classInfo) => [classInfo.class_id, getClassRangeStats(classInfo)]));

  const overallStats = classes.length > 0 ? (() => {
    const classStatsList = classes
      .map((c) => classStatsById.get(c.class_id))
      .filter((s): s is NonNullable<typeof s> => Boolean(s));

    const totalPresent = classStatsList.reduce((sum, s) => sum + s.present, 0);
    const totalAbsent = classStatsList.reduce((sum, s) => sum + s.absent, 0);

    const classesWithSessions = classStatsList.filter((s) => s.totalClasses > 0);
    const avgAttendance = classesWithSessions.length > 0
      ? classesWithSessions.reduce((sum, s) => sum + s.percentage, 0) / classesWithSessions.length
      : 0;
    ...
    <div className="min-h-screen h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-blue-50 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-md border border-emerald-200">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
              <div>
                ...
                <p className="text-sm sm:text-base text-slate-600">{rangeLabel}</p>
              </div>
              <div className="w-full sm:w-auto flex justify-end">
                <MonthYearSelector
                  ...
                  onRangeChange={({ startMonth, startYear, endMonth, endYear }) => {
          ...
          {error && (
            <div className="mt-4 mb-4 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">
              {error}
            </div>
          )}
          {overallStats && (
          ...
          {classes.length === 0 ? (
            ...
            <div className={shouldScrollClasses ? 'max-h-[60vh] overflow-y-auto pr-1' : ''}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {filteredClasses.map((classInfo) => {
                  const stats = classStatsById.get(classInfo.class_id) || {
                    present: 0,
                    absent: 0,
                    late: 0,
                    totalClasses: 0,
                      ...
                      <div className="p-4 md:p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center">
                          ...
                          <div className="p-2 bg-rose-50 rounded-lg">
                            ...
                            <p className="text-lg font-bold text-rose-700">{stats.absent}</p>
                          </div>
