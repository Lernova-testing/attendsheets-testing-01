...
function includesStatusCode(error: unknown, code: string): boolean {
...
export async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`Server error on attempt ${attempt + 1}`, {
        url,
      ...
      if (
        includesStatusCode(error, '404') ||
        includesStatusCode(error, '401') ||
        includesStatusCode(error, '403')
      ) {
        throw error;
      }

      console.warn(`Network error on attempt ${attempt + 1}`, {
        url,
